...
final class ImageToVideoCommand extends Command
{
     ...
     */
    public const INTROS = [
        ['🎬', 'Bringing your picture to life with {model}…', 'Films take their time — perfect for a pot of tea and some cookies 🍪'],
        ['🌬️', 'Adding motion to your image with {model}…', 'Grab a warm drink and a snack; frames take a while ☕'],
        ['🎞️', 'Drawing frame after frame from your image with {model}…', 'Cozy up with some cookies — this takes patience 🍪'],
        ['📽️', 'Animating your picture with {model}…', 'How about a hot chocolate while the scene comes together? ☕'],
    ];
    ...
    protected const USAGE = <<<'TXT'
        ...
        Arguments:
          model                   Hugging Face model id, e.g. stabilityai/stable-video-diffusion-img2vid-xt
                                  (pull it first)
          image                   Image file to animate, e.g. photo.jpg

        Options:
          --output=PATH           Video file to write; its extension picks the format: .mp4, .webm, .mkv or .gif
                                  (default: a timestamped .mp4 in 'output_dir' in config/image-to-video.php)
          --prompt=TEXT           Text describing the video, for models that take one; models that animate the
                                  image on their own, such as Stable Video Diffusion, refuse it
          ...
          --log-file=PATH         Append the runner's output to this file (default: 'log_file' in config/image-to-video.php)
    ...
    /**
     * @param ImageToVideoConfig|null   $config    defaults to the package's config/image-to-video.php
    ...
    public function __construct(
        private ?ImageToVideoConfig $config = null,
        $stdout = null,
        $stderr = null,
    ...
    protected function execute(array $positional, array $options): int
    {
        ...
        $config = $this->config ??= ImageToVideoConfig::load();

        $runner = new ImageToVideo($this->storage);
        ...
        $this->startLog($options['log-file'] ?? $config->logFile, "image-to-video {$model}: {$image}");
        ...
        try {
            ...
            return $this->binaryFailed("Error: failed to animate {$image} with {$model} (exit code {$e->exitCode}).");
        }
