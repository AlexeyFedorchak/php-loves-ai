"""Generate a video from a text prompt with a locally pulled Hugging Face diffusers pipeline.
...
Works with diffusers text-to-video pipelines, e.g. Wan, LTX-Video, CogVideoX or AnimateDiff.
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="text-to-video",
        description="Generate a video from a text prompt with a locally pulled diffusers pipeline.",
    )
...
def main(argv: list[str]) -> int:
    ...
    try:
        ...
        log(f"[text-to-video] Loading {args.model} on {device}...")
        ...
        log("[text-to-video] Generating video...")
        result = pipeline(prompt=args.prompt, **options)
        ...
        log(f"[text-to-video] Error: {e}")
        return EXIT_FAILED
    except Exception as e:
        traceback.print_exc()
        log(f"[text-to-video] Error: {type(e).__name__}: {e}")
        return EXIT_FAILED

    print(json.dumps({"output": output, "frames": len(frames), "width": width, "height": height, "fps": args.fps}), flush=True)
    log(f"[text-to-video] Saved {output} ({len(frames)} frames, {width}x{height}).")
    return EXIT_OK
